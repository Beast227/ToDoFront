...
export default function UserProfile({ onClose }: UserProfileProps) {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        ...
        const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/users/data`, {
          method: "GET",
          headers: {
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    const payload = {
        ...
        email: userData.email, // Sent back but not editable in UI
        ...(newPassword && { password: newPassword }),
        ...(currentPassword && { currentPassword }) // Send current PW if your backend requires verification
    };
          ...
          <form onSubmit={handleUpdate} className="space-y-4">
            ...
            <div className="grid gap-3">
                <input
                    ...
                    className={`w-full bg-gray-950 border rounded-lg px-3 py-2 text-white placeholder-gray-600 focus:ring-1 focus:border-cyan-500 outline-none transition-all ${
                        confirmPassword && newPassword !== confirmPassword ? "border-red-500 focus:border-red-500 focus:ring-red-500" : "border-gray-700 focus:ring-cyan-500"
                    }`}
                />
            </div>
